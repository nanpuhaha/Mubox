		...
		private void InputThreadMain()
		{
			try
			{
				...
				while (!_exitYet)
				{
					...
					if (_inputQueue.Count > 0)
					{
						//lock (_inputQueueThreadLock)
						{
							...
							try
							{
								...
								while (_inputQueue.TryDequeue(out input))
								{
					...
					// every iteration waits at least 20ms before continue, on lower end systems this makes a difference
					var waitTimeMilliseconds = 20 - stopwatch.ElapsedMilliseconds;
					if (waitTimeMilliseconds > 0)
					{
						waiter.WaitOne((int)waitTimeMilliseconds);
					}
				}
			}
			catch (Exception ex)
