...
function X4D_DB:Open(database, version)
    ...
    if (database._name == nil) then
        database._name = "TRANSIENT" .. tostring(math.floor(debugprofilestop()))
    end
	local proto = {
		_table = database
	}
    ...
    for key,entity in pairs(self._table) do
        if (type(entity) == "table") then
			if (predicate == nil) then
	            return entity, key
			elseif (predicate(entity, key)) then
	            return entity, key
			end
        end
    end
...
SLASH_X4DB1 = "/x4db"
SlashCmdList["X4DB"] = function (parameters, other)
X4D.Log:Debug({parameters})
