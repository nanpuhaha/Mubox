local X4D_Log = LibStub:NewLibrary("X4D_Log", 2000)
if (not X4D_Log) then
	return
end
...
--TODO: add global hook for 'print()' so we can wrap print() calls with a timestamp, and also queue it during init (when DEFAULT_CHAT_FRAME is not yet ready)
...
X4D:RegisterForEvent("X4D", "PLAYER_ENTERING_WORLD", function()
    _initialized = true
    TryFlushMessageBuffer()
end)
